...
(java -cp integration/build/libs/flute-integration-all-0.1.2.jar -Djava.net.preferIPv4Stack=true com.aitusoftware.flute.integration.service.DatabaseServerMain $DB_DIR) < /dev/null > build/logs/db-server.log 2> build/logs/db-server.log &
(java -XX:+UnlockCommercialFeatures -XX:+FlightRecorder -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5005 -cp persistor/build/libs/flute-persistor-all-0.1.2.jar  -Djava.net.preferIPv4Stack=true -Dlog4j.configuration=file://`pwd`/persistor/src/main/resources/logging/log4j.xml -Dflute.db.metrics.url=$DB_URL com.aitusoftware.flute.archive.FluteMetricsPersistorMain $CONFIG_FILE) < /dev/null > build/logs/persistor.log 2> build/logs/persistor.log &
(java -XX:+UnlockCommercialFeatures -XX:+FlightRecorder -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5006 -cp server/build/libs/flute-server-all-0.1.2.jar  -Djava.net.preferIPv4Stack=true -Dlog4j.configuration=file://`pwd`/server/src/main/resources/logging/log4j.xml -Dflute.db.metrics.url=$DB_URL -Dflute.db.reports.url=$REPORT_DB_URL -Dflute.server.httpPort=$HTTP_SERVER_PORT -Dflute.resource.base=server/src/main/resources/ui com.aitusoftware.flute.server.http.HttpQueryServerMain $CONFIG_FILE)  < /dev/null > build/logs/http-server.log 2> build/logs/http-server.log &
