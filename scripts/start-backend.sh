...
(java -XX:+UnlockCommercialFeatures -XX:+FlightRecorder -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5006 -cp flute-server/build/libs/flute-server-all-0.1.4.jar  -Djava.net.preferIPv4Stack=true -Dlog4j.configuration=file://`pwd`/flute-server/src/main/resources/logging/log4j.xml -Dflute.db.metrics.url=$DB_URL -Dflute.db.reports.url=$REPORT_DB_URL -Dflute.server.httpPort=$HTTP_SERVER_PORT com.aitusoftware.flute.server.http.HttpQueryServerMain $CONFIG_FILE)  < /dev/null > build/logs/metrics-server.log 2> build/logs/metrics-server.log &
(java -XX:+UnlockCommercialFeatures -XX:+FlightRecorder -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5007 -cp flute-server/build/libs/flute-server-all-0.1.4.jar  -Djava.net.preferIPv4Stack=true -Dlog4j.configuration=file://`pwd`/flute-server/src/main/resources/logging/log4j.xml -Dflute.db.metrics.url=$DB_URL -Dflute.db.reports.url=$REPORT_DB_URL -Dflute.server.httpPort=$REPORTING_HTTP_SERVER_PORT com.aitusoftware.flute.server.reporting.http.ReportAdminServerMain $CONFIG_FILE)  < /dev/null > build/logs/report-admin-server.log 2> build/logs/report-admin-server.log &
